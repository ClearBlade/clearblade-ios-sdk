...
static void on_connect(struct mosquitto *mosq, void *obj, int rc)
{
    ...
    [[client controller] didConnect:(NSUInteger)rc];
...
{
    MosquittoClient* client = (__bridge MosquittoClient*)obj;
    [[client controller] didDisconnect];
}

static void on_publish(struct mosquitto *mosq, void *obj, int message_id)
{
    MosquittoClient* client = (__bridge MosquittoClient*)obj;
    [[client controller] didPublish:(NSUInteger)message_id];
}
...
static void on_message(struct mosquitto *mosq, void *obj, const struct mosquitto_message *message)
{
    ...
    [[client controller] didReceiveMessage:mosq_msg];
...
static void on_subscribe(struct mosquitto *mosq, void *obj, int message_id, int qos_count, const int *granted_qos)
{
    ...
    [[client controller] didSubscribe:message_id grantedQos:nil];
}

static void on_unsubscribe(struct mosquitto *mosq, void *obj, int message_id)
{
    MosquittoClient* client = (__bridge MosquittoClient*)obj;
    [[client controller] didUnsubscribe:message_id];
}
...
- (MosquittoClient*) initWithClientId: (NSString*) clientId Controller: (id) ctrl {
    if ((self = [super init])) {
        const char* cstrClientId = [clientId cStringUsingEncoding:NSUTF8StringEncoding];
        [self setHost: nil];
        [self setPort: 1883];
